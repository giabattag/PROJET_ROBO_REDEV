function [thrust, moments] = droneControlPID(u, err_i, p, v, q, omega, control_type_param, drone_params, DroneID, g)
...
global kp_position_pid kd_position_pid ki_position_pid kp_attitude_pid kd_attitude_pid % ki_attitude_pid

m = drone_params(DroneID).mechanical.m;
kp_attitude = kp_attitude_pid;
kd_attitude = kd_attitude_pid;
% ki_attitude = ki_attitude_pid;
...
elseif control_type_param == 3
  ...
  ad = u(1:3); % desired velocity in flat body frame
  yrd = u(4); % desired yawrate
  [~,y]= build_flat_rotation(q); % rotation of quadrotor in world frame
  ad = quat2rotm(q')*ad; % express desired acceleration in world frame
  fd = saturateThrust(ad,drone_params(DroneID).control.limits.thrust.max, m, g);
  thrust = norm(fd);
  moments = fullAttitudeControl(fd,y,q,omega,kp_attitude,kd_attitude); % in world frame
  moments(3) =0.1*(moments(3) + kp_attitude*(yrd-omega(3)));
